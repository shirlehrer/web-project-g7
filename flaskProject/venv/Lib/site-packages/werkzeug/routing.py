...
.. code-block:: python
    ...
    m = Map([
        # Static URLs
        Rule('/', endpoint='css/index'),
        Rule('/about', endpoint='css/about'),
        Rule('/help', endpoint='css/help'),
        # Knowledge Base
        Subdomain('kb', [
...
.. code-block:: python
    ...
    c.build("css/index", force_external=True)
...
def parse_rule(rule: str) -> t.Iterator[t.Tuple[t.Optional[str], t.Optional[str], str]]:
    ...
    `None` it's a css url part, otherwise it's a dynamic one.
...
class Rule(RuleFactory):
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI application (like css data)
    ...
    def match_compare_key(
        ...
        2.  rules with more css parts come first so the second argument
            is the negative length of the number of the css weights.
        3.  we order by css weights, which is a combination of index
            and length
